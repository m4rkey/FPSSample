...
public class GameMode1v1 : IGameMode
{
    [ConfigVar(Name = "game.1v1.minplayers", DefaultValue = "2", Description = "Minimum players before match starts")]
    public static ConfigVar minPlayers;
    [ConfigVar(Name = "game.1v1.prematchtime", DefaultValue = "20", Description = "Time before match starts")]
    public static ConfigVar preMatchTime;
    [ConfigVar(Name = "game.1v1.postmatchtime", DefaultValue = "10", Description = "Time after match ends before new will begin")]
    public static ConfigVar postMatchTime;
    [ConfigVar(Name = "game.1v1.roundlength", DefaultValue = "30", Description = "Round length (seconds)")]
    ...
    public void Initialize(GameWorld world, GameModeSystemServer gameModeSystemServer)
    {
        ...
        Console.Write("1on1 game mode initialized");
    }
    ...
    public void Restart()
    {
        ...
        m_GameModeSystemServer.StartGameTimer(preMatchTime, "Warmup");
    }
        ...
        switch (m_Phase)
        {
            case Phase.Countdown:
                if (m_GameModeSystemServer.GetGameTimer() == 0)
                {
                    ...
                    {
                        m_GameModeSystemServer.chatSystem.SendChatAnnouncement("Waiting for opponent.");
                        m_GameModeSystemServer.StartGameTimer(preMatchTime, "Warmup");
                    }
                    else
                    {
            ...
            case Phase.Active:
                ...
                // On game tie, just reset, nobody won.
                if (m_GameModeSystemServer.GetGameTimer() == 0)
                {
                    ...
                    m_GameModeSystemServer.StartGameTimer(postMatchTime, "Post Round");
    ...
    public void OnPlayerKilled(PlayerState victim, PlayerState killer)
    {
        ...
        if (killer != null)
        {
            ...
            {
                // Killer wins, give him a round and restart.

                killer.score++;
                m_GameModeSystemServer.teams[killer.teamIndex].score++;

                // TODO: Instead of killing everyone on round end, lets just passively respawn them after x seconds.
                var players = m_GameModeSystemServer.playersComponentGroup.GetComponentArray<PlayerState>();
                for (int i = 0, c = players.Length; i < c; i++)
                {
                    var playerState = players[i];
                    ...
                    playerState.gameResult = (playerState.teamIndex == killer.teamIndex) ? "VICTORY" : "DEFEAT";
                    //playerState.displayScoreBoard = false;
                    //playerState.displayGoal = false;
                }
                ...
                var l = 0;
                l = StringFormatter.Write(ref _msgBuf, 0, "Round over. <winner should go here>");
                m_GameModeSystemServer.chatSystem.SendChatAnnouncement(new CharBufView(_msgBuf, l));
            }
        }
    }
